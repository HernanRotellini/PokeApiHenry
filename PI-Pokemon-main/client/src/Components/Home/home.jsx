...
function Home(props) {
  ...
  const handleTypeFilterChange = (event) => {
    ...
    dispatch(
      filteredPokemons({ type: event.target.value, origin: originFilter })
    );
    if (alphabeticOrder !== "NoOrder")
      dispatch(orderedPokemons(alphabeticOrder));
    if (attackOrder !== "NoOrder") dispatch(orderedPokemons(attackOrder));
  };

  const handleOriginFilterChange = (event) => {
    setOriginFilter(event.target.value);
    dispatch(
      filteredPokemons({ type: typeFilter, origin: event.target.value })
    );
    if (alphabeticOrder !== "NoOrder")
      dispatch(orderedPokemons(alphabeticOrder));
    if (attackOrder !== "NoOrder") dispatch(orderedPokemons(attackOrder));
  ...
  return (
    <div>
      ...
      <div className={style.cards}>
        {loading ? (
          <img
            src="https://i.postimg.cc/pdtFRpqh/Loading-Pokemon.gif"
            alt="Cargando..."
